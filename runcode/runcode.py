...
class RunPyCode(object):
    ...
    def extract_input_output_names(self, code):
        ...
        regex = r"@input( ?)=( ?)(\w+)( ?);( ?)@output( ?)=( ?)(\w+)"

        # ioline = "# @input= a ; @output =b\n"  # will work with all space combinations
        matches = re.findall(regex, ioline)
        match = matches[0] # we have only one match, if we have many need to you iterate
        input, output = str(match[2]), str(match[7])
